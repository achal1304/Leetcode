...
func sumTraversal(root *TreeNode, arr []int, index int) []int {
	if root == nil {
		return arr
	}
	...
	arr = sumTraversal(root.Left, arr, index+1)
	arr = sumTraversal(root.Right, arr, index+1)
	return arr
}
